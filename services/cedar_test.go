...
func TestDialCedar(t *testing.T) {
	...
	t.Run("ConnectToCedar", func(t *testing.T) {
		conn, err := DialCedar(ctx, client, httpAddress, rpcAddress, username, password, 10)
	...
	t.Run("IncorrectUsernameAndPassword", func(t *testing.T) {
		conn, err := DialCedar(ctx, client, httpAddress, rpcAddress, "bad_user", "bad_password", 10)
		assert.Error(t, err)
		assert.Nil(t, conn)
	})
}
